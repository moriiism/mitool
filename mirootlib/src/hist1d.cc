...
void HistData1d::GetXRangeQdp(double* const low_ptr,
                              double* const up_ptr) const
{
    double low, up;
    MxkwMath::GetRangeQdp(GetXvalLo(), GetXvalUp(), &low, &up);
    *low_ptr = low;
    *up_ptr  = up;
    if(0 < g_flag_verbose){
        MPrintInfo("done.");
...
void HistData1d::GetORangeQdp(double* const low_ptr,
                              double* const up_ptr) const
{
    double low, up;
    MxkwMath::GetRangeQdp(GetOvalArr()->GetValMin(),
...
DataArray1d* const HistData1d::GenRandomEvt(int rand_seed) const
{
    TRandom3* trand = new TRandom3(rand_seed);
    vector<double> time_vec;

    double tbinfwidth = GetBinWidth();
    for(long ibin = 0; ibin < nbin_xval_; ibin ++){
        double bin_center = GetBinCenterX(ibin);
...
Interval* const HistData1d::GenIntervalAboveThreshold(double threshold) const
{
    ...
    for(long ibin = 0; ibin < nbin_xval_; ibin ++){
        if(GetOvalElm(ibin) > threshold){
            tstart_vec.push_back(GetBinLoX(ibin));
            tstop_vec.push_back(GetBinUpX(ibin));
        }
    }
...
int HistData1d::IsHi1dNotNull() const
{
    if(NULL == GetHi1d()){
        abort();
    }
...
int HistData1d::IsOvalArrNotNull() const
{
    if(NULL == GetOvalArr()){
        abort();
    }
