                             ...
                             double* const add_err_ptr) const
{
    double add = 0.0;
    double add_err = 0.0;
    MirMath::GetAdd(GetNdata(), GetVal(), GetValSerr(), &add, &add_err);
                               ...
                               vector<long>* const index_bad_vec_ptr) const
{
    ...
    vector<long> index_bad_vec;
    long num_bad = MirMath::GetWMean(GetNdata(), GetVal(), GetValSerr(),
...
{
    double ans = MirMath::GetAMean(GetNdata(), GetValSerr());
    return ans;
}
...
///    long num_bad = MirMath::GetChi2byConst(GetNdata(), GetVal(), GetValSerr(),
