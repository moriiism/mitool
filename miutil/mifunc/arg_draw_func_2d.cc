...
void ArgValDrawFunc2d::Init(int argc, char* argv[])
{
    progname_ = "draw_func_2d";
...
void ArgValDrawFunc2d::Print(FILE* fp) const
{
...
void ArgValDrawFunc2d::SetOption(int argc, char* argv[], option* long_options)
{
    if(0 < g_flag_verbose){
        MPrintInfo("start...");
    }
...
void ArgValDrawFunc2d::Usage(FILE* fp) const
{
    fprintf(fp,
            "usage: %s [--help (0)] [--verbose (0)] [--debug (0)] "
            "[--plotmode (on)/off] [--printmode (on)/off] [--root_style (def)/std/pub/detail] "
            "[--npoint_func (500)] "
            "func_name  par_file  plot_conf_projx_file  plot_conf_projy_file "
            "outdir  outfile_head\n",
            progname_.c_str());
    abort();
}
