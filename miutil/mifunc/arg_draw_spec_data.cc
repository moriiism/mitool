...
void ArgValDrawSpecData::Init(int argc, char* argv[])
{
    progname_ = "draw_spec_data";
...
void ArgValDrawSpecData::Print(FILE* fp) const
{
...
void ArgValDrawSpecData::SetOption(int argc, char* argv[], option* long_options)
{
    if(0 < g_flag_verbose){
        MPrintInfo("start...");
    }
...
void ArgValDrawSpecData::Usage(FILE* fp) const
{
    fprintf(fp,
            "usage: %s [--help (0)] [--verbose (0)] [--debug (0)] "
            "[--plotmode (on)/off] [--printmode (on)/off] [--root_style (def)/std/pub/detail] "
            "data_file  data_fmt  func_name  par_file  spec_type  plot_conf_file  outdir  outfile_head\n",
            progname_.c_str());
    abort();
}
