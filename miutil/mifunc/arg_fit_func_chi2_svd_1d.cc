#include "arg_fit_func_chi2_svd_1d.h"

// public

void ArgValFitFuncChi2Svd1d::Init(int argc, char* argv[])
{
    progname_ = "fit_func_chi2_svd_1d";
...
void ArgValFitFuncChi2Svd1d::Print(FILE* fp) const
{
    ...
    fprintf(fp, "%s: stat_           : %s\n", __func__, stat_.c_str());
}

// private

void ArgValFitFuncChi2Svd1d::Null()
{
    progname_ = "";
    data_file_ = "";
    data_fmt_ = "";
...
void ArgValFitFuncChi2Svd1d::SetOption(int argc, char* argv[], option* long_options)
{
    if(0 < g_flag_verbose){
        MPrintInfo("start...");
    }
    ...
    while (1) {
        ...
        case 0:
            ...
            if(0 == strcmp("npoint_func", long_options[option_index].name) ){
                ...
                root_style_ = optarg;
            } else if (0 == strcmp("stat", long_options[option_index].name) ){
                stat_ = optarg;
            }
            break;
        case 'd':
...
void ArgValFitFuncChi2Svd1d::Usage(FILE* fp) const
{
    fprintf(fp,
            ...
            "[--stat (gauss)/poisson] "
            "data_file  data_fmt  func_par_list  plot_conf_file  "
            "outdir  outfile_head\n",
            progname_.c_str());
    abort();
}
