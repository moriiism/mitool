...
void ArgValFitFuncCstatUnbinEm1d::Init(int argc, char* argv[])
{
    progname_ = "fit_func_cstat_unbin_em_1d";
...
void ArgValFitFuncCstatUnbinEm1d::Print(FILE* fp) const
{
...
void ArgValFitFuncCstatUnbinEm1d::Null()
{
    progname_ = "";
    data_file_ = "";
    func_par_list_ = "";
...
void ArgValFitFuncCstatUnbinEm1d::SetOption(int argc, char* argv[], option* long_options)
{
    if(0 < g_flag_verbose){
        MPrintInfo("start...");
    }
...
void ArgValFitFuncCstatUnbinEm1d::Usage(FILE* fp) const
{
    fprintf(fp,
            "usage: %s [--help (0)] [--verbose (0)] [--debug (0)] "            
            "[--plotmode (on)/off] [--printmode (on)/off] [--root_style (def)/std/pub/detail] "
            "[--npoint_func (500)] "
            "data_file  func_par_list  hist_mask  "
