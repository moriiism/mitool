...
void ArgValDrawSpecDataList::Init(int argc, char* argv[])
{
    progname_ = "draw_spec_data_list";
    ...
    spec_type_      = argv[iarg]; iarg++;    
    plot_conf_file_ = argv[iarg]; iarg++;
    outdir_         = argv[iarg]; iarg++;
    outfile_head_   = argv[iarg]; iarg++;
}

void ArgValDrawSpecDataList::Print(FILE* fp) const
{
    ...
    fprintf(fp, "%s: spec_type_    : %s\n", __func__, spec_type_.c_str());    
...
void ArgValDrawSpecDataList::Null()
{
    progname_ = "";
    data_file_list_ = "";
    func_par_list_ = "";
...
void ArgValDrawSpecDataList::SetOption(int argc, char* argv[], option* long_options)
{
    if(0 < g_flag_verbose){
        MPrintInfo("start...");
    }
...
void ArgValDrawSpecDataList::Usage(FILE* fp) const
{
    fprintf(fp,
            "usage: %s [--help (0)] [--verbose (0)] [--debug (0)] "
            "[--plotmode (on)/off] [--printmode (on)/off] [--root_style (def)/std/pub/detail] "
            "data_file_list  func_par_list  spec_type  plot_conf_file  outdir  outfile_head\n",
            progname_.c_str());
    abort();
}
