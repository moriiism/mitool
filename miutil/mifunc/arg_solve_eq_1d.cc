...
void ArgValSolveEq1d::Init(int argc, char* argv[])
{
    progname_ = "solve_eq_1d";
    ...
    constant_       = atof(argv[iarg]); iarg++;
    plot_conf_file_ = argv[iarg]; iarg++;
    outdir_         = argv[iarg]; iarg++;
    outfile_head_   = argv[iarg]; iarg++;
}

void ArgValSolveEq1d::Print(FILE* fp) const
{
...
void ArgValSolveEq1d::SetOption(int argc, char* argv[], option* long_options)
{
    if(0 < g_flag_verbose){
        MPrintInfo("start...");
    }
...
void ArgValSolveEq1d::Usage(FILE* fp) const
{
    fprintf(fp,
            "usage: %s [--help (0)] [--verbose (0)] [--debug (0)] "
            "[--plotmode (on)/off] [--printmode (on)/off] [--root_style (def)/std/pub/detail] "
            "func_name  par_file  constant  plot_conf_file  outdir  outfile_head\n",
            progname_.c_str());
    abort();
}
