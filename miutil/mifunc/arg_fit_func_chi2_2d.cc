#include "arg_fit_func_chi2_2d.h"

// public

void ArgValFitFuncChi22d::Init(int argc, char* argv[])
{
    progname_ = "fit_func_chi2_2d";
...
void ArgValFitFuncChi22d::Print(FILE* fp) const
{
...
void ArgValFitFuncChi22d::SetOption(int argc, char* argv[], option* long_options)
{
    if(0 < g_flag_verbose){
        MPrintInfo("start...");
    }
...
void ArgValFitFuncChi22d::Usage(FILE* fp) const
{
    fprintf(fp,
            "usage: %s [--help (0)] [--verbose (0)] [--debug (0)] "
            "[--plotmode (on)/off] [--printmode (on)/off] [--root_style (def)/std/pub/detail] "
            "data_file  data_fmt  func_name  fitpar  chi2par  chi2cont_par  "
