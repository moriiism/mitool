...
void ArgValDrawFuncData1d::Init(int argc, char* argv[])
{
    progname_ = "draw_func_data_1d";
...
void ArgValDrawFuncData1d::Print(FILE* fp) const
{
...
void ArgValDrawFuncData1d::Null()
{
    progname_       = "";
    data_file_      = "";
    data_fmt_       = "";
...
void ArgValDrawFuncData1d::SetOption(int argc, char* argv[], option* long_options)
{
    if(0 < g_flag_verbose){
        MPrintInfo("start...");
    }
...
void ArgValDrawFuncData1d::Usage(FILE* fp) const
{
    fprintf(fp,
            "usage: %s [--help (0)] [--verbose (0)] [--debug (0)] "
            "[--npoint_func (500)] "
            "[--plotmode (on)/off] [--printmode (on)/off] [--root_style (def)/std/pub/detail] "
            "data_file  data_fmt  func_name  par_file  plot_conf_file  outdir  outfile_head\n",
            progname_.c_str());
    abort();
}
