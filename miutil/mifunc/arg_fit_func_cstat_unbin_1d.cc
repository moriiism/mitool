...
void ArgValFitFuncCstatUnbin1d::Init(int argc, char* argv[])
{
    progname_ = "fit_func_cstat_unbin_1d";
...
void ArgValFitFuncCstatUnbin1d::Print(FILE* fp) const
{
...
void ArgValFitFuncCstatUnbin1d::Null()
{
    progname_ = "";
    data_file_ = "";
    func_name_ = "";
...
void ArgValFitFuncCstatUnbin1d::SetOption(int argc, char* argv[], option* long_options)
{
    if(0 < g_flag_verbose){
        MPrintInfo("start...");
    }
...
void ArgValFitFuncCstatUnbin1d::Usage(FILE* fp) const
{
    fprintf(fp,
            "usage: %s [--help (0)] [--verbose (0)] [--debug (0)] "            
            "[--plotmode (on)/off] [--printmode (on)/off] [--root_style (def)/std/pub/detail] "
            "[--npoint_func (500)] "
            "data_file  func_name  hist_mask  fitpar  chi2par  chi2cont_par  minfcn_name  "
