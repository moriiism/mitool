...
void ArgValFitFuncCstatUnbinQpMulti1d::Init(int argc, char* argv[])
{
    progname_ = "fit_func_cstat_unbin_qp_multi_1d";
    ...
    func_par_id_list_  = argv[iarg]; iarg++;
    func_id_list_      = argv[iarg]; iarg++;
    tie_coeff_list_    = argv[iarg]; iarg++;
    plot_conf_file_    = argv[iarg]; iarg++;
    outdir_            = argv[iarg]; iarg++;
    outfile_head_      = argv[iarg]; iarg++;
}

void ArgValFitFuncCstatUnbinQpMulti1d::Print(FILE* fp) const
{
    ...
    fprintf(fp, "%s: func_par_id_list_ : %s\n", __func__, func_par_id_list_.c_str());
    fprintf(fp, "%s: func_id_list_     : %s\n", __func__, func_id_list_.c_str());
    fprintf(fp, "%s: tie_coeff_list_   : %s\n", __func__, tie_coeff_list_.c_str());
...
void ArgValFitFuncCstatUnbinQpMulti1d::Null()
{
    progname_ = "";
    data_file_list_ = "";
    func_par_id_list_ = "";
...
void ArgValFitFuncCstatUnbinQpMulti1d::SetOption(int argc, char* argv[], option* long_options)
{
    if(0 < g_flag_verbose){
        MPrintInfo("start...");
    }
...
void ArgValFitFuncCstatUnbinQpMulti1d::Usage(FILE* fp) const
{
    fprintf(fp,
            "usage: %s [--help (0)] [--verbose (0)] [--debug (0)] "            
            "[--plotmode (on)/off] [--printmode (on)/off] [--root_style (def)/std/pub/detail] "
            "[--npoint_func (500)] "
            "data_file_list  func_par_id_list  func_id_list  tie_coeff_list  "
