...
void ArgValFitFuncCstat2d::Init(int argc, char* argv[])
{
    progname_ = "fit_func_cstat_2d";
...
void ArgValFitFuncCstat2d::Print(FILE* fp) const
{
...
void ArgValFitFuncCstat2d::Null()
{
    progname_ = "";
    data_file_ = "";
    data_fmt_ = "";
...
void ArgValFitFuncCstat2d::SetOption(int argc, char* argv[], option* long_options)
{
    if(0 < g_flag_verbose){
        MPrintInfo("start...");
    }
...
void ArgValFitFuncCstat2d::Usage(FILE* fp) const
{
    fprintf(fp,
            "usage: %s [--help (0)] [--verbose (0)] [--debug (0)] "
            "[--plotmode (on)/off] [--printmode (on)/off] [--root_style (def)/std/pub/detail] "
            "data_file  data_fmt  func_name  fitpar  chi2par  chi2cont_par  minfcn_name_ "
