...
void ArgValDrawFuncDataList1d::Init(int argc, char* argv[])
{
    progname_ = "draw_func_data_list_1d";
...
void ArgValDrawFuncDataList1d::Print(FILE* fp) const
{
...
void ArgValDrawFuncDataList1d::Null()
{
    progname_ = "";
    data_file_list_ = "";
    func_par_list_ = "";
...
void ArgValDrawFuncDataList1d::SetOption(int argc, char* argv[], option* long_options)
{
    if(0 < g_flag_verbose){
        MPrintInfo("start...");
    }
...
void ArgValDrawFuncDataList1d::Usage(FILE* fp) const
{
    fprintf(fp,
            "usage: %s [--help (0)] [--verbose (0)] [--debug (0)] "
            "[--plotmode (on)/off] [--printmode (on)/off] [--root_style (def)/std/pub/detail] "
            "data_file_list  func_par_list  plot_conf_file  outdir  outfile_head\n",
            progname_.c_str());
    abort();
}
