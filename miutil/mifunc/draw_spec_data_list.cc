...
#include "func_user.h"
#include "arg_draw_spec_data_list.h"
...
int main(int argc, char* argv[]){
    ...
    ArgValDrawSpecDataList* argval = new ArgValDrawSpecDataList;
    ...
    FuncSpec** func_arr = new FuncSpec* [nfunc];
    TF1** tf1_spec_arr  = new TF1* [nfunc];
    for(int ifunc = 0; ifunc < nfunc; ifunc ++){
        printf("ifunc = %d\n", ifunc);
        ...
        if("f_nu" == argval->GetSpecType()){
            tf1_spec_arr[ifunc] = func_arr[ifunc]->GenTF1FNu(func_range_lo, func_range_up);
        } else if ("nu_f_nu" == argval->GetSpecType()){
            tf1_spec_arr[ifunc] = func_arr[ifunc]->GenTF1NuFNu(func_range_lo, func_range_up);
        } else if ("ph_nu" == argval->GetSpecType()){
            tf1_spec_arr[ifunc] = func_arr[ifunc]->GenTF1PhNu(func_range_lo, func_range_up);
        } else {
            MPrintErr("bad spec type");
            abort();
        }
        MirFuncPar::CopyPar(func_par, tf1_spec_arr[ifunc]);
        tf1_spec_arr[ifunc]->SetLineStyle(func_line_style_arr[ifunc]);
        tf1_spec_arr[ifunc]->SetLineColor(func_line_color_arr[ifunc]);
        tf1_spec_arr[ifunc]->Draw("SAME");
        tf1_spec_arr[ifunc]->Draw("sameaxis");
    }

    char outfig_nufnu[kLineSize];
    sprintf(outfig_nufnu, "%s_spec.png", argval->GetOutfileHead().c_str());
    root_tool->GetTCanvas()->Update();
    root_tool->GetTCanvas()->Print(outfig_nufnu);

    for(int ifunc = 0; ifunc < nfunc; ifunc ++){
        delete func_arr[ifunc];
        delete tf1_spec_arr[ifunc];
    }
    delete [] func_arr;
    delete [] tf1_spec_arr;
