...
void ArgValPow::Init(int argc, char* argv[])
{
    progname_ = "pow";
    ...
    powspec_type_ = argv[iarg];       iarg++;
    outdir_       = argv[iarg];       iarg++;
    outfile_head_ = argv[iarg];       iarg++;
}

void ArgValPow::Print(FILE* fp) const
{
    ...
    fprintf(fp, "%s: tbinfwidth_    : %e\n", __func__, tbinfwidth_);
    fprintf(fp, "%s: powspec_type_  : %s\n", __func__, powspec_type_.c_str());
...
void ArgValPow::SetOption(int argc, char* argv[], option* long_options)
{
    if(0 < g_flag_verbose){
        MPrintInfo("start...");
    }
